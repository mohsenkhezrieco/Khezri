...
%                   = 2, mix between sar and spatial Durbin model                     
%--------------------------------------------------- 
...
[junk nvartot]=size(results.xwith);
mix=results.mix; % mix is number of X variables without WX specification. 
% It is assumed that these variables are specified first and does not
% include the intercept
if (cflag==1) nvar=(nvartot-mix-1)/2; else nvar=(nvartot-mix)/2; end
if (cflag==1) nvartotc=nvartot-1; else nvartotc=nvartot; end
NSIM=1000;
simresults=zeros(nvartotc+1,NSIM);
simdir=zeros(mix+nvar,NSIM);
simind=zeros(mix+nvar,NSIM);
simtot=zeros(mix+nvar,NSIM);
for sim=1:NSIM
    parms = chol(results.cov)'*randn(size(parm)) + parm;
    rhosim = parms(nvartot+1,1);
    if (results.cflag==1) betasim=parms(2:nvartot,1);
